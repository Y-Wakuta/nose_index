...
create_db(){
  recho "create db"
  cqlsh -e "create keyspace if not exists $1 with REPLICATION = {'class': 'SimpleStrategy','replication_factor': 3}" $cass_db_ip
  check_dbs $1
}

drop_db(){
  recho "drop db"
  cqlsh -e "drop keyspace if exists $1" $cass_db_ip
  check_dbs $1
}
...
exec_bench(){
  ...
  recho "sleeping"
  sleep 200
  bundle exec nose benchmark schemas/$1.json --format csv > TOD/bench_result_$1.csv
  drop_db $1
  sleep 200
}

echo "run nose_index si"
#exec_bench rubis_secondary_bidding_161000000_si
bundle exec nose benchmark schemas/rubis_secondary_bidding_161000000_si.json --format csv > TOD/bench_result_rubis_secondary_bidding_161000000_si.csv
echo "run nose_index no si"
