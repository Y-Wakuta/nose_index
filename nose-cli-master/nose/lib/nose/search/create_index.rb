...
class CreateIndex
  ...
  def get_has_index_hash(indexes)
    ...
    table_name = ""
    fields = Set.new
    has_index =  indexes.map do |index|
      mySQL_table_name = get_MySQL_table_name_by_index(index)
      next {key: index.key,has_index: false} if !is_replace_2nd_index index #yusuke 単体のindexとして置き換えるメリットが無い場合、弾く
      ...
      if table_name != mySQL_table_name then
        ...
        next {key: index.key,has_index: false}
      end
      ...
      next {key:index.key,has_index: true} #yusuke 上記の条件を全てクリアした場合、index作成
    end
    has_index
  end
end
