    ...
    class SecondaryIndexPresenceConstraints < Constraint
      ...
      def self.apply(problem)
        problem.indexes.select{|si| si.is_secondary_index}.each do |si|
          base_cf_indexes = problem.indexes.select{|cf| !cf.is_secondary_index && cf.hash_fields == si.extra and (cf.order_fields.to_set + cf.extra) >= si.hash_fields} 
          if base_cf_indexes.empty? 
