...
up_baseline(){
  ...
  recho "sleeping"
  sleep 100
  bundle exec nose benchmark plans/rubis_baseline.rb --format csv > rubis_baseline_bench_result_real_rubis_data.csv
  drop_db baseline
  sleep 100
}
...
exec_bench(){
  ...
  recho "sleeping"
  sleep 100
  bundle exec nose benchmark schemas/$1.json --format csv > bench_result_$1_realrubis_data.csv
  drop_db $1
  sleep 100
}
